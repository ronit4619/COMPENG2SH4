...
void testConstructor1Invalid3(){
	int invalid_row2 = 1, invalid_col2 =1;
	string expected = "0 \n";
	Matrix b = Matrix(invalid_row2,invalid_col2);
	ASSERT_EQUAL(expected,b.toString());
}
...
void testConstructor2_new() {
	  // for constructor2
	    int row=4,col=5;
        int data[4][5] = {{1,2,3,4,5},{6,7,8,9,0},{8,7,6,5,5},{4,3,2,1,0}};
        int*input_data[4];
        for (int i=0;i<4;i++){
        	input_data[i]=data[i];
        }
...
void testIsUpperTrTrue_new() {
		//test isUpperTr()

	  int row=4,col=3;
      int upper1[4][3] = {{1,2,3},{0,4,3},{0,0,1},{0,0,0}};
	  int*input_upper1[4];
	  for (int i=0;i<4;i++){
	    	input_upper1[i]=upper1[i];
	  }
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(testConstructor2_new));
	s.push_back(CUTE(testGetElementValid_new));
	s.push_back(CUTE(testSetElementValid_new));
	s.push_back(CUTE(testCopy_new));
	s.push_back(CUTE(testIsUpperTrTrue_new));
