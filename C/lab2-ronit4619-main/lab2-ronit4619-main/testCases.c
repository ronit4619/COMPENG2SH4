...
void TestQ1_norm_negative(CuTest *tc) {
	...
	int n = 2;
	double input1[2] = {3.60, 4.78, -6.00, 10.00, 0.01};
	double expected=13.108 ;
    double actual = norm2(input1,n);

    CuAssertDblEquals(tc, expected, actual,0.009);
}
void TestQ1_add_negative(CuTest *tc) {

	int n = 2;
	double input1[2] = {1.00,-2.00};
	double input2[2] = {-2.00,1.00};
	double actual[2];
	double expected [2] = {-1.00,-1.00};
	add_vectors(input1,input2,actual,n);
	int i;
	for (i=0; i<n; i++)
		CuAssertDblEquals(tc, expected[i], actual[i],0.009);
...
void TestQ1_scalar_prod_negative(CuTest *tc) {
...
void TestQ2_true_null(CuTest *tc) {
...
void TestQ3_double_digit(CuTest *tc) {
	int n = 3;
	int input[3][3] = {{10, 20, 30},{ 50, 80, 9},{ 10, 30, 50}};
	int expected[9]= {10, 50, 20, 10, 80, 30, 30, 90, 50};
	int actual[9];
	diag_scan(input,actual);
...
void TestQ4_no_letter(CuTest *tc) {
